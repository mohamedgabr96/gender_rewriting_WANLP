...
def main(config):
    ...
    #         })
    all_all_sentences = []
    for direction in ["MM", "FM", "MF", "FF"]:
        config.speak_listen_mode = direction
        ...
        all_all_sentences += all_new_sentences
    ...
    p, r, f1 = calculate_m2(all_all_sentences, config.m2_edits_path, split='dev')
    path_to_result_json = os.path.join(config.output_save_location, f"results_alldirs_scores.json")
    with open(path_to_result_json, encoding="utf-8", mode="w") as file_opened:
        json_to_dump = {
            "Precision": p,
            "Recall": r,
