...
def load_normal_data(path, split="train", direction="MF", augmented=False, use_target_prefix=False):
    ...
    with open(os.path.join(path, split, f"{split}.arin.label" + augmented_string), encoding="utf-8") as file_opened:
        source_prefix_labels = file_opened.readlines()

    source_prefix_labels = [x.replace("</s>", "").replace("</s>", "").replace("<s>", "").strip("\n") for x in source_prefix_labels]
    added_target = ""
    if use_target_prefix:
        added_target = "-" + direction.strip("\n").replace("</s>", "")
    
    source = [y + added_target + " " + x.replace("</s>", "").replace("</s>", "").replace("<s>", "").strip("\n") for x, y in zip(source, source_prefix_labels)]
    ...
    target = [x.replace("</s>", "").replace("</s>", "").replace("<s>", "").strip("\n") for x in target]
