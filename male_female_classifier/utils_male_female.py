...
def load_normal_data_speaker_listener(path, split="train", direction="MF", augmented=False):
    ...
    for line in source:
        if line.strip(" ") == '\n':
            ...
            if len(temp_line[2+augmented_remove]) == 4 and (temp_line[2+augmented_remove][1] == "M" or temp_line[2+augmented_remove][1] == "F"):
                word = temp_line[1+augmented_remove]
                label = 1 if temp_line[2+augmented_remove][1] == "M" else 2
            else:
                word = temp_line[1+augmented_remove]
                label = 0
